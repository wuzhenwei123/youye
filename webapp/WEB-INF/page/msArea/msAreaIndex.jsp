...
<body style="background:#eee;">
	...
	<div id="content1">
	  <div id="content-header">
		      ...
		      <div class="widget-box">
		        ...
		        <div class="widget-content nopadding collapse" id="search-coll">
		          <form class="form-horizontal">
	                ...
	                <div class="control-group name_title">
		                <label class="control-label name_label">areaCode:</label>
	                	<div class="controls name_control">
			                <input type="text" class="span8 name_span" id="fAreaCode"/>
			            </div>
	                </div>
	                <div class="control-group name_title">
		                <label class="control-label name_label">areaName:</label>
	                	<div class="controls name_control">
			                <input type="text" class="span8 name_span" id="fAreaName"/>
			            </div>
	                </div>
	                <div class="control-group name_title">
		...
		<perm:tag permPath="/msArea/addMsArea" >
	    <button class="btn btn-success" onclick="toAdd()"><i class="icon-plus"></i>添加</button>
		</perm:tag>
		<perm:tag permPath="/msArea/removeAllMsArea" >
	...
	<script type="text/javascript">
		...
		function searchData(pageNo){
		    ...
		    var alp = $("#fAlp").val();
		    $.getJSON("<c:url value='/msArea/getMsAreaList'/>",
	        {
		...
		function genTableHeader(){
			var str = "<thead><tr>" ;
		    	...
		    	str+= "<th class=\"sortTh\" id=\"th_areaCode\" column='areaCode'>areaCode<i class=\"icon-sort\"></i></th>";
		    	str+= "<th class=\"sortTh\" id=\"th_areaName\" column='areaName'>areaName<i class=\"icon-sort\"></i></th>";
		...
		function setTableStr(result, pageNo, returnNum){
		    ...
		    for (var k=0; k<result.items.length; k++){      
		        ...
		        tableStr += "<td>";
	        	tableStr += "<perm:tag permPath='/msArea/updateMsArea' ><button class=\"btn btn-mini btn-info\" onclick='toUpdate(" + result.items[k].id + ");'><i class=\"icon-edit\"></i> 编辑</button></perm:tag>";
	        	tableStr += "<perm:tag permPath='/msArea/removeMsArea' ><button class=\"btn btn-mini btn-danger\" onclick='del(" + result.items[k].id + ");'><i class=\"icon-trash\"></i>删除</button></perm:tag>";
	        	tableStr += "</td>";
		        tableStr += "</tr>";            
		    }
		    tableStr += "</tbody>";
		...
		// 添加
 		function toAdd(){
   			top.art.dialog.open('<c:url value="/msArea/toAdd"/>',
			{
				id:456,
				fixed:true,
				esc:true,
   		...
   		function add(iframe){
			...
			var areaCode = getIframeVal(iframe,"aAreaCode");
			var areaName = getIframeVal(iframe,"aAreaName");
			var cityCode = getIframeVal(iframe,"aCityCode");
			var alp = getIframeVal(iframe,"aAlp");
			var flag = true;
		    if (flag){ 
		        $.post("<c:url value='/msArea/addMsArea'/>",
		        	{
		    		id : id,
		    		areaCode : areaCode,
		    		areaName : areaName,
		...
		function toUpdate(id) {
			top.art.dialog.open('<c:url value="/msArea/toUpdate?id='+id+'"/>',
			{
				id:123,
				fixed:true,
				title:'msArea',
		...
		// 执行更新
   		function update(iframe){
			...
			var areaCode = getIframeVal(iframe,"mAreaCode");
			var areaName = getIframeVal(iframe,"mAreaName");
			var cityCode = getIframeVal(iframe,"mCityCode");
			var alp = getIframeVal(iframe,"mAlp");
			var flag = true;
		    if (flag){ 
		        $.post("<c:url value='/msArea/updateMsArea'/>",
		        	{
		    		id : id,
		    		areaCode : areaCode,
		    		areaName : areaName,
   		...
   		function delAll(){
   		    ...
   		    if(ids == ''){
   		    	...
   		    	parent.layer.msg('你确定要删除吗？', {
  	   			  ...
  	   			  ,btn: ['确定', '取消']
  	   			  ,yes: function(index){
  	   				$.post("<c:url value='/msArea/removeAllMsArea'/>",
		        	{
						ids :ids,
						ranNum:Math.random()},
			        	function(data){
		...
		function del(id){
   		   if (id != ""){ 
	   			parent.layer.msg('你确定要删除吗？', {
	   			  ...
	   			  ,btn: ['确定', '取消']
	   			  ,yes: function(index){
	   			 	$.post("<c:url value='/msArea/removeMsArea'/>",
   		        	{
   						id	:id,
   						ranNum:Math.random()
   					},
