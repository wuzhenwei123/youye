...
<body style="background:#eee;">
	...
	<div id="content1">
	  <div id="content-header">
		      ...
		      <div class="widget-box">
		        ...
		        <div class="widget-content nopadding collapse" id="search-coll">
		          <form class="form-horizontal">
	                ...
	                <div class="control-group name_title">
		                <label class="control-label name_label">provinceName:</label>
	                	<div class="controls name_control">
			                <input type="text" class="span8 name_span" id="fProvinceName"/>
			            </div>
	                </div>
	                <div class="control-group name_title">
		...
		<perm:tag permPath="/msProvince/addMsProvince" >
	    <button class="btn btn-success" onclick="toAdd()"><i class="icon-plus"></i>添加</button>
		</perm:tag>
		<perm:tag permPath="/msProvince/removeAllMsProvince" >
	...
	<script type="text/javascript">
		...
		function searchData(pageNo){
		    ...
		    var alp = $("#fAlp").val();
		    $.getJSON("<c:url value='/msProvince/getMsProvinceList'/>",
	        {
		...
		function genTableHeader(){
			var str = "<thead><tr>" ;
		    	...
		    	str+= "<th class=\"sortTh\" id=\"th_provinceName\" column='provinceName'>provinceName<i class=\"icon-sort\"></i></th>";
		...
		function setTableStr(result, pageNo, returnNum){
		    ...
		    for (var k=0; k<result.items.length; k++){      
		        ...
		        tableStr += "<td>";
	        	tableStr += "<perm:tag permPath='/msProvince/updateMsProvince' ><button class=\"btn btn-mini btn-info\" onclick='toUpdate(" + result.items[k].id + ");'><i class=\"icon-edit\"></i> 编辑</button></perm:tag>";
	        	tableStr += "<perm:tag permPath='/msProvince/removeMsProvince' ><button class=\"btn btn-mini btn-danger\" onclick='del(" + result.items[k].id + ");'><i class=\"icon-trash\"></i>删除</button></perm:tag>";
	        	tableStr += "</td>";
		        tableStr += "</tr>";            
		    }
		    tableStr += "</tbody>";
		...
		// 添加
 		function toAdd(){
   			top.art.dialog.open('<c:url value="/msProvince/toAdd"/>',
			{
				id:456,
				fixed:true,
				esc:true,
   		...
   		function add(iframe){
			...
			var provinceName = getIframeVal(iframe,"aProvinceName");
			var countryCode = getIframeVal(iframe,"aCountryCode");
			var alp = getIframeVal(iframe,"aAlp");
			var flag = true;
		    if (flag){ 
		        $.post("<c:url value='/msProvince/addMsProvince'/>",
		        	{
		    		id : id,
		    		provinceCode : provinceCode,
		    		provinceName : provinceName,
		...
		function toUpdate(id) {
			top.art.dialog.open('<c:url value="/msProvince/toUpdate?id='+id+'"/>',
			{
				id:123,
				fixed:true,
				title:'msProvince',
		...
		// 执行更新
   		function update(iframe){
			...
			var provinceName = getIframeVal(iframe,"mProvinceName");
			var countryCode = getIframeVal(iframe,"mCountryCode");
			var alp = getIframeVal(iframe,"mAlp");
			var flag = true;
		    if (flag){ 
		        $.post("<c:url value='/msProvince/updateMsProvince'/>",
		        	{
		    		id : id,
		    		provinceCode : provinceCode,
		    		provinceName : provinceName,
   		...
   		function delAll(){
   		    ...
   		    if(ids == ''){
   		    	...
   		    	parent.layer.msg('你确定要删除吗？', {
  	   			  ...
  	   			  ,btn: ['确定', '取消']
  	   			  ,yes: function(index){
  	   				$.post("<c:url value='/msProvince/removeAllMsProvince'/>",
		        	{
						ids :ids,
						ranNum:Math.random()},
			        	function(data){
		...
		function del(id){
   		   if (id != ""){ 
	   			parent.layer.msg('你确定要删除吗？', {
	   			  ...
	   			  ,btn: ['确定', '取消']
	   			  ,yes: function(index){
	   			 	$.post("<c:url value='/msProvince/removeMsProvince'/>",
   		        	{
   						id	:id,
   						ranNum:Math.random()
   					},
