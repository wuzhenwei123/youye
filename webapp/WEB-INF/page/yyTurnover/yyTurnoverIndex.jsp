...
<body style="background:#fff;">
	...
	<div id="content1">
	  <div id="content-header">
		        ...
		        <div class="widget-content nopadding collapse" id="search-coll">
		          <form class="form-horizontal">
	                <div class="control-group name_title">
		                <label class="control-label name_label">营业额范围:</label>
		...
		<perm:tag permPath="/yyTurnover/addYyTurnover" >
	    <button class="btn btn-success" onclick="toAdd()"><i class="icon-plus"></i>添加</button>
		</perm:tag>
		<perm:tag permPath="/yyTurnover/removeAllYyTurnover" >
	...
	<script type="text/javascript">
		...
		function searchData(pageNo){
		    ...
		    $.getJSON("<c:url value='/yyTurnover/getYyTurnoverList'/>",
	        {
		...
		function genTableHeader(){
			var str = "<thead><tr>" ;
		    	...
		    	str+= "<th>序号</th>";
		    	str+= "<th class=\"sortTh\" id=\"th_name\" column='name'>营业额范围</th>";
		...
		function setTableStr(result, pageNo, returnNum){
		    ...
		    for (var k=0; k<result.items.length; k++){      
		        ...
		        tableStr += "<td>";
	        	tableStr += "<perm:tag permPath='/yyTurnover/updateYyTurnover' ><button class=\"btn btn-mini btn-info\" onclick='toUpdate(" + result.items[k].id + ");'><i class=\"icon-edit\"></i> 编辑</button></perm:tag>";
	        	tableStr += "<perm:tag permPath='/yyTurnover/removeYyTurnover' ><button class=\"btn btn-mini btn-danger\" onclick='del(" + result.items[k].id + ");'><i class=\"icon-trash\"></i>删除</button></perm:tag>";
	        	tableStr += "</td>";
		        tableStr += "</tr>";            
		    }
		    tableStr += "</tbody>";
		...
		// 添加
 		function toAdd(){
   			top.art.dialog.open('<c:url value="/yyTurnover/toAdd"/>',
			{
				id:456,
				fixed:true,
				esc:true,
   		...
   		function add(iframe){
			...
			var flag = true;
		    if (flag){ 
		        $.post("<c:url value='/yyTurnover/addYyTurnover'/>",
		        	{
		    		id : id,
		    		name : name,
		    		state : state,
		...
		function toUpdate(id) {
			top.art.dialog.open('<c:url value="/yyTurnover/toUpdate?id='+id+'"/>',
			{
				id:123,
				fixed:true,
				title:'编辑营业额范围',
		...
		// 执行更新
   		function update(iframe){
			...
			var flag = true;
		    if (flag){ 
		        $.post("<c:url value='/yyTurnover/updateYyTurnover'/>",
		        	{
		    		id : id,
		    		name : name,
		    		state : state,
   		...
   		function delAll(){
   		    ...
   		    if(ids == ''){
   		    	...
   		    	parent.layer.msg('你确定要删除吗？', {
  	   			  ...
  	   			  ,btn: ['确定', '取消']
  	   			  ,yes: function(index){
  	   				$.post("<c:url value='/yyTurnover/removeAllYyTurnover'/>",
		        	{
						ids :ids,
						ranNum:Math.random()},
			        	function(data){
		...
		function del(id){
   		   if (id != ""){ 
	   			parent.layer.msg('你确定要删除吗？', {
	   			  ...
	   			  ,btn: ['确定', '取消']
	   			  ,yes: function(index){
	   			 	$.post("<c:url value='/yyTurnover/removeYyTurnover'/>",
   		        	{
   						id	:id,
   						ranNum:Math.random()
   					},
