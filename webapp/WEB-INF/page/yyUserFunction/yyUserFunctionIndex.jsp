...
<body style="background:#fff;">
	...
	<div id="content1">
	  <div id="content-header">
		...
		<perm:tag permPath="/yyUserFunction/addYyUserFunction" >
	    <button class="btn btn-success" onclick="toAdd()"><i class="icon-plus"></i>添加</button>
		</perm:tag>
		<perm:tag permPath="/yyUserFunction/removeAllYyUserFunction" >
	...
	<script type="text/javascript">
		...
		function searchData(pageNo){
		    ...
		    $.getJSON("<c:url value='/yyUserFunction/getYyUserFunctionList'/>",
	        {
		...
		function setTableStr(result, pageNo, returnNum){
		    ...
		    for (var k=0; k<result.items.length; k++){      
		        ...
		        tableStr += "<td>";
	        	tableStr += "<perm:tag permPath='/yyUserFunction/updateYyUserFunction' ><button class=\"btn btn-mini btn-info\" onclick='toUpdate(" + result.items[k].id + ");'><i class=\"icon-edit\"></i> 编辑</button></perm:tag>";
	        	tableStr += "<perm:tag permPath='/yyUserFunction/removeYyUserFunction' ><button class=\"btn btn-mini btn-danger\" onclick='del(" + result.items[k].id + ");'><i class=\"icon-trash\"></i>删除</button></perm:tag>";
	        	tableStr += "</td>";
		        tableStr += "</tr>";            
		    }
		    tableStr += "</tbody>";
		...
		// 添加
 		function toAdd(){
   			top.art.dialog.open('<c:url value="/yyUserFunction/toAdd"/>',
			{
				id:456,
				fixed:true,
				esc:true,
				title:'添加yyUserFunction',
				width: '730px',
				height:'400px',
			    cancelVal: '取消',
   		...
   		function add(iframe){
			...
			var flag = true;
		    if (flag){ 
		        $.post("<c:url value='/yyUserFunction/addYyUserFunction'/>",
		        	{
		    		id : id,
		    		user_id : user_id,
		    		function_id : function_id,
		...
		function toUpdate(id) {
			top.art.dialog.open('<c:url value="/yyUserFunction/toUpdate?id='+id+'"/>',
			{
				id:123,
				fixed:true,
				title:'yyUserFunction',
				esc:true,
				width: '730px',
				height:'400px',
			    cancelVal: '取消',
		...
		// 执行更新
   		function update(iframe){
			...
			var flag = true;
		    if (flag){ 
		        $.post("<c:url value='/yyUserFunction/updateYyUserFunction'/>",
		        	{
		    		id : id,
		    		user_id : user_id,
		    		function_id : function_id,
   		...
   		function delAll(){
   		    ...
   		    if(ids == ''){
   		    	...
   		    	parent.layer.msg('你确定要删除吗？', {
  	   			  ...
  	   			  ,btn: ['确定', '取消']
  	   			  ,yes: function(index){
  	   				$.post("<c:url value='/yyUserFunction/removeAllYyUserFunction'/>",
		        	{
						ids :ids,
						ranNum:Math.random()},
			        	function(data){
		...
		function del(id){
   		   if (id != ""){ 
	   			parent.layer.msg('你确定要删除吗？', {
	   			  ...
	   			  ,btn: ['确定', '取消']
	   			  ,yes: function(index){
	   			 	$.post("<c:url value='/yyUserFunction/removeYyUserFunction'/>",
   		        	{
   						id	:id,
   						ranNum:Math.random()
   					},
