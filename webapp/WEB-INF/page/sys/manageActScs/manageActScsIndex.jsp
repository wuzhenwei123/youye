		        ...
		        <div class="widget-content nopadding collapse" id="search-coll">
		          <form class="form-horizontal">
	                <div class="control-group name_title">
		                <label class="control-label name_label">请求路径:</label>
	                	<div class="controls name_control">
			                <input type="text" class="span8 name_span" id="fRequestPath"/>
			            </div>
	                </div>
<!-- 	                <div class="control-group name_title"> -->
<!-- 		                <label class="control-label name_label">请求次数:</label> -->
<!-- 	                	<div class="controls name_control"> -->
...
<%-- 		<perm:tag permPath="/manageActScs/addManageActScs" > --%>
<!-- 	    <button class="btn btn-success" onclick="toAdd()"><i class="icon-plus"></i>添加</button> -->
<%-- 		</perm:tag> --%>
<%-- 		<perm:tag permPath="/manageActScs/removeAllManageActScs" > --%>
...
<%-- 		</perm:tag> --%>
	...
	<script type="text/javascript">
		...
		function searchData(pageNo){
		    ...
		    var id = $("#fId").val();
		    var requestPath = $("#fRequestPath").val();
		    ...
		    $.getJSON("<c:url value='/manageActScs/getManageActScsList'/>",
	        {
...
// 			        tableStr += "<td>" + result.items[k].id + "</td>";
			        tableStr += "<td>" + result.items[k].requestPath + "</td>";
			        tableStr += "<td>" + result.items[k].requestCount + "</td>";
			        tableStr += "<td>" + result.items[k].requestDateTime + "</td>";
// 		        tableStr += "<td>";
// 	        	tableStr += "<perm:tag permPath='/manageActScs/updateManageActScs' ><button class=\"btn btn-mini btn-info\" onclick='toUpdate(" + result.items[k].id + ");'><i class=\"icon-edit\"></i> 编辑</button></perm:tag>";
// 	        	tableStr += "<perm:tag permPath='/manageActScs/removeManageActScs' ><button class=\"btn btn-mini btn-danger\" onclick='del(" + result.items[k].id + ");'><i class=\"icon-trash\"></i>删除</button></perm:tag>";
// 	        	tableStr += "</td>";
		        tableStr += "</tr>";            
		    }
		    tableStr += "</tbody>";
		...
		// 添加
 		function toAdd(){
   			top.art.dialog.open('<c:url value="/manageActScs/toAdd"/>',
			{
				id:456,
				fixed:true,
				esc:true,
				title:'添加manageActScs',
				width: '730px',
				height:'400px',
			    cancelVal: '取消',
   		...
   		function add(iframe){
			...
			var requestPath = getIframeVal(iframe,"aRequestPath");
			var requestCount = getIframeVal(iframe,"aRequestCount");
			var requestDateTime = getIframeVal(iframe,"aRequestDateTime");
			var flag = true;
		    if (flag){ 
		        $.post("<c:url value='/manageActScs/addManageActScs'/>",
		        	{
		    		id : id,
		    		requestPath : requestPath,
		    		requestCount : requestCount,
		...
		function toUpdate(id) {
			top.art.dialog.open('<c:url value="/manageActScs/toUpdate?id='+id+'"/>',
			{
				id:123,
				fixed:true,
				title:'manageActScs',
		...
		// 执行更新
   		function update(iframe){
			...
			var requestPath = getIframeVal(iframe,"mRequestPath");
			var requestCount = getIframeVal(iframe,"mRequestCount");
			var requestDateTime = getIframeVal(iframe,"mRequestDateTime");
			var flag = true;
		    if (flag){ 
		        $.post("<c:url value='/manageActScs/updateManageActScs'/>",
		        	{
		    		id : id,
		    		requestPath : requestPath,
		    		requestCount : requestCount,
   		...
   		function delAll(){
   		    ...
   		    if(ids == ''){
   		    	...
   		    	parent.layer.msg('你确定要删除吗？', {
  	   			  ...
  	   			  ,btn: ['确定', '取消']
  	   			  ,yes: function(index){
  	   				$.post("<c:url value='/manageActScs/removeAllManageActScs'/>",
		        	{
						ids :ids,
						ranNum:Math.random()},
			        	function(data){
		...
		function del(id){
   		   if (id != ""){ 
	   			parent.layer.msg('你确定要删除吗？', {
	   			  ...
	   			  ,btn: ['确定', '取消']
	   			  ,yes: function(index){
	   			 	$.post("<c:url value='/manageActScs/removeManageActScs'/>",
   		        	{
   						id	:id,
   						ranNum:Math.random()
   					},
