...
<body style="background:#fff;">
	...
	<script type="text/javascript">
		...
		function searchData(){
			...
			$.getJSON("${ctx}/yyDataAnalysisController/userStudyAnalysis",
	    		{
					year_str:year_str,
					_t: Math.random()
		        },function(data){
	            ...
	            if (result.code == 1) {
	            	...
	            	var option = "{title: {text: '"+json.year_str+"年用户平均每月学习时长',left: '50%'},color: ['#1eb4ec'],";
	            	option = option + "tooltip : {trigger: 'axis'},toolbox: {feature: {saveAsImage: {}}},";
	            	option = option + "grid: {left: '3%',right: '4%',bottom: '3%',containLabel: true},";
	            	option = option + "xAxis : [{type : 'category',boundaryGap : false,data : ["+json.mouthY_str+"]}],";
	            	option = option + "yAxis : [{type : 'value'}],";
	            	option = option + "series : [{name:'学习时长（分钟）',type:'line',label: {normal: {show: true,position: 'top'}},";
	            	option = option + "data:["+json.data_str+"]}]}";
