...
<body style="background:#fff;">
	...
	<div id="content1">
	  <div id="content-header">
		        ...
		        <div class="widget-content nopadding collapse" id="search-coll">
		          <form class="form-horizontal">
		          	<div class="control-group name_title">
		                <label class="control-label name_label">工具名称:</label>
	                	<div class="controls name_control">
			                <input type="text" class="span8 name_span" id="fName"/>
			            </div>
	                </div>
	                <div class="control-group name_title">
		                <label class="control-label name_label">关联课程名称:</label>
	                	<div class="controls name_control">
			                <input type="text" class="span8 name_span" id="fCourse_name"/>
			            </div>
	                </div>
	                <div class="control-group name_title">
		...
		<perm:tag permPath="/yyCourseAppendix/addYyCourseAppendix" >
	    <button class="btn btn-success" onclick="toAdd()"><i class="icon-plus"></i>添加</button>
		</perm:tag>
		<perm:tag permPath="/yyCourseAppendix/removeAllYyCourseAppendix" >
	...
	<script type="text/javascript">
		...
		function searchData(pageNo){
		    ...
		    var url = $("#fUrl").val();
		    var name = $("#fName").val();
		    var course_name = $("#fCourse_name").val();
		    $.getJSON("<c:url value='/yyCourseAppendix/getYyCourseAppendixList'/>",
	        {
		...
		function genTableHeader(){
			var str = "<thead><tr>" ;
		    	...
		    	str+= "<th>序号</th>";
		    	str+= "<th class=\"sortTh\" id=\"th_name\" column='name'>工具名称</th>";
		    	str+= "<th class=\"sortTh\" id=\"th_course_id\" column='course_id'>关联课程</th>";
		    	str+= "<th class=\"sortTh\" id=\"th_url\" column='url'>状态</th>";
				str+="<th>操作</th>";
				str+="</tr></thead>";
			return str;
		}
		function setTableStr(result, pageNo, returnNum){
		    ...
		    for (var k=0; k<result.items.length; k++){      
		        ...
		        tableStr += "<td >" + (number + k + 1) + "</td>";
			        ...
			        if(result.items[k].course_id!=""){
			        	tableStr += "<td>" + result.items[k].course_name + "</td>";
			        }else{
			        	tableStr += "<td>暂无关联</td>";
			        }
		        ...
		        tableStr += "<td>";
	        	tableStr += "<perm:tag permPath='/yyCourseAppendix/updateYyCourseAppendix' ><button class=\"btn btn-mini btn-info\" onclick='toUpdate(" + result.items[k].id + ");'><i class=\"icon-edit\"></i> 编辑</button></perm:tag>";
	        	tableStr += "<perm:tag permPath='/yyCourseAppendix/removeYyCourseAppendix' ><button class=\"btn btn-mini btn-danger\" onclick='del(" + result.items[k].id + ");'><i class=\"icon-trash\"></i>删除</button></perm:tag>";
	        	tableStr += "</td>";
		        tableStr += "</tr>";            
		    }
		    tableStr += "</tbody>";
		...
		// 添加
 		function toAdd(){
   			top.art.dialog.open('<c:url value="/yyCourseAppendix/toAdd"/>',
			{
				id:456,
				fixed:true,
				esc:true,
   		...
   		function add(iframe){
			...
			var url = getIframeVal(iframe,"aUrl");
			var name = getIframeVal(iframe,"aName");
			var state = getIframeVal(iframe,"aState");
			var flag = true;
		    if (flag){ 
		        $.post("<c:url value='/yyCourseAppendix/addYyCourseAppendix1'/>",
		        	{
		    		id : id,
		    		course_id : course_id,
		    		url : url,
		...
		function toUpdate(id) {
			top.art.dialog.open('<c:url value="/yyCourseAppendix/toUpdate?id='+id+'"/>',
			{
				id:123,
				fixed:true,
				title:'编辑工具',
		...
		// 执行更新
   		function update(iframe){
			...
			var url = getIframeVal(iframe,"mUrl");
			var name = getIframeVal(iframe,"mName");
			var state = getIframeVal(iframe,"mState");
			var flag = true;
		    if (flag){ 
		        $.post("<c:url value='/yyCourseAppendix/updateYyCourseAppendix'/>",
		        	{
		    		id : id,
		    		course_id : course_id,
		    		url : url,
   		...
   		function delAll(){
   		    ...
   		    if(ids == ''){
   		    	...
   		    	parent.layer.msg('你确定要删除吗？', {
  	   			  ...
  	   			  ,btn: ['确定', '取消']
  	   			  ,yes: function(index){
  	   				$.post("<c:url value='/yyCourseAppendix/removeAllYyCourseAppendix'/>",
		        	{
						ids :ids,
						ranNum:Math.random()},
			        	function(data){
		...
		function del(id){
   		   if (id != ""){ 
	   			parent.layer.msg('你确定要删除吗？', {
	   			  ...
	   			  ,btn: ['确定', '取消']
	   			  ,yes: function(index){
	   			 	$.post("<c:url value='/yyCourseAppendix/removeYyCourseAppendix'/>",
   		        	{
   						id	:id,
   						ranNum:Math.random()
   					},
